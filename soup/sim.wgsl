...
struct Params { n: u32, gw: u32, gh: u32, epochs: u32, cap: u32, seed: u32, mrate: u32, grid_local: u32, ext_ops: u32, };
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  for (var ep: u32 = 0u; ep < P.epochs; ep = ep + 1u) {
    ...
    loop {
      ...
      else if (op == 0x9au) { let nb = nbrBase(cell, x, y, hs & 3u); cells[nb + h1] = t[h0]; }               // FAR (writer dst=h1)
      else if (op == 0x4cu) { hs = (hs + 1u) & 255u; }
      else if (op == 0x4du) { hs = (hs + 255u) & 255u; }
      else if (P.ext_ops == 1u && op == 0x50u) { let nb = nbrBase(cell, x, y, hs & 3u); cells[nb + h0] = t[h0]; }   // SCATTER (position-locked dst=h0)
      else if (P.ext_ops == 1u && op == 0x51u) { let nb = ((cell + 1u) % P.n) * STRIDE; cells[nb + h1] = t[h0]; }    // MATE (flat neighbour)
      else if (P.ext_ops == 1u && op == 0x52u) { let nb = nbrBase(cell, x, y, hs & 3u); t[h0] = cells[nb + h0] & 0xffu; dirty[h0 >> 5u] = dirty[h0 >> 5u] | (1u << (h0 & 31u)); }  // GATHER (read neighbour)
      // else: NOP
